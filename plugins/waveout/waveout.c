...
#define DEFAULT_BUFFER_SIZE_STR "dummy"
...
static int wave_terminate = 0;
static char thread_wait = 0;
static WAVEFORMATEXTENSIBLE wave_format;
// -buffers
static int audio_blocks_sent = 0; // remaining blocks
static int audio_block_offset = 0; // current block
...
static int
pwaveout_openstream (void);

static int
pwaveout_closestream (void);

int
pwaveout_init (void) {
    // we can't relay on deadbeef calling this
    trace("pwaveout_init\n");
...
static unsigned int
get_waveout_device () {
    unsigned int new_wdevice = waveout_device;
    deadbeef->conf_lock ();
    const char *current_dev = deadbeef->conf_get_str_fast ("waveout_soundcard", "default");
    if (strcmp(current_dev, "default") == 0) {
        ...
        deadbeef->conf_unlock ();
        new_wdevice = (unsigned int) deadbeef->conf_get_int ("waveout_soundcard", -1);
    }
    return new_wdevice;
}
...
waveout_message (uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
    ...
    switch (id) {
        case DB_EV_CONFIGCHANGED:
            new_wdevice = get_waveout_device();
            trace ("new_wdevice: %d\n",new_wdevice);
            if ((waveout_device != new_wdevice) && device_handle) {
...
static int
pwaveout_openstream () {
    // start stream with shiny clean setup
    trace ("pwaveout_openstream\n");

    /* handle the format change... */
    waveout_device = get_waveout_device ();
    MMRESULT mmresult = waveOutOpen(&device_handle, waveout_device, (WAVEFORMATEX *)&wave_format, (DWORD_PTR)waveOutProc, 0, CALLBACK_FUNCTION);
    if (mmresult != MMSYSERR_NOERROR)
    {
        /* FATAL! cannot continue playback */
        trace("pwaveout_openstream: failed to open stream (result=%d)\n",mmresult);
    ...
    else
    {
        ...
        audio_block_offset = 0;
        trace("pwaveout_openstream: opening stream done (bytes_per_block=%d)\n",bytes_per_block);
    }
    return 0;
}
...
static int
pwaveout_closestream () {
    // stop stream, clear buffers etc.
    trace ("pwaveout_closestream\n");
    if (device_handle != NULL) {
        EnterCriticalSection(&waveoutCS);
        waveOutReset(device_handle);

        int i;
        for (i=0; i<AUDIO_BUFFER_NO; i++)
        {
            /* free resources still locked */
            if (waveout_headers[i].dwFlags & WHDR_PREPARED)
                waveOutUnprepareHeader(device_handle, &waveout_headers[i], sizeof(WAVEHDR));
        }
        /* close the device */
        waveOutClose(device_handle);
        device_handle = NULL;
        audio_blocks_sent = 0;
        audio_block_offset = 0; // current block
        bytesread = 0;
        //bytes_per_block
        avail_audio_buffers = 0;
...
pwaveout_setformat (ddb_waveformat_t *fmt) {
    ...
    if (!memcmp (&plugin.fmt, fmt, sizeof (ddb_waveformat_t))) {
        trace ("pwaveout_setformat ignored\n");
        return 0;
    }
    ...
    int result = 0;
    {
        MMRESULT openresult = waveOutOpen(NULL, waveout_device, (WAVEFORMATEX *)&wave_format, 0, 0, WAVE_FORMAT_QUERY);
...
pwaveout_play (void) {
    ...
    trace("pwaveout_play\n");
    if (!device_handle) {
        ret = pwaveout_openstream ();
        if (ret) {
            trace ("failed to open stream\n");
            return -1;
        }
    }
    if (!waveout_tid) {
...
pwaveout_thread (void *context)
{
    ...
    while (1)
    {
        ...
        if (state != OUTPUT_STATE_PLAYING || thread_wait)
        {
            //trace ("nanosleep\n");
        ...
        else
        {
            ...
            if (deadbeef->streamer_ok_to_read(bytes_per_block)) {
                bytesread = deadbeef->streamer_read(audio_data+audio_block_offset*bytes_per_block, bytes_per_block);
                if (bytesread > 0) {
                    /* I cannot "unprepare" into the callback, so this seems a good place to do it */
                    EnterCriticalSection(&waveoutCS);
                    if (waveout_headers[audio_block_offset].dwFlags & WHDR_PREPARED)
                        waveOutUnprepareHeader(device_handle, &waveout_headers[audio_block_offset], sizeof(WAVEHDR));
                    waveout_headers[audio_block_offset].dwBufferLength = bytesread;
                    waveout_headers[audio_block_offset].lpData =         audio_data+audio_block_offset*bytes_per_block;
                    waveout_headers[audio_block_offset].dwFlags =        0;

                    mmresult = waveOutPrepareHeader(device_handle, &waveout_headers[audio_block_offset], sizeof(WAVEHDR));
                    if (mmresult == MMSYSERR_NOERROR)
                    {
                        waveOutWrite(device_handle, &waveout_headers[audio_block_offset], sizeof(WAVEHDR));
                        audio_blocks_sent++;
                        audio_block_offset = (audio_block_offset+1) % avail_audio_buffers;
                    }
                    LeaveCriticalSection(&waveoutCS);
                }
                else {
...
pwaveout_enumdevices (void (*callback)(const char *name, const char *desc, void *), void *userdata) {
    ...
    char * charset = 0;
    int devenc_list = deadbeef->conf_get_int ("waveOut.devenc_list", 0);
    if (devenc_list == 0)
        ...
        if (!charset)
            return;
        deadbeef->conf_get_str ("waveout.devenc_custom", "", charset, 255);
    }
    if (charset)
        trace ("waveout: converting device names from charset %s\n", charset);
    ...
    for (idx=0; idx<num_devices; idx++)
    {
        ...
        {
            // convert name
            char * name_converted = woc.szPname;
            char name_converted_allocated = 0;
            if (charset && strlen(charset)) {
                name_converted = malloc (strlen(woc.szPname) * 4);
                name_converted_allocated = 1;
                deadbeef->junk_iconv (woc.szPname, strlen(woc.szPname), name_converted, strlen(woc.szPname)*4, charset, "UTF-8//IGNORE");
            }

            {
                char num[8];
                snprintf (num, 8, "%d", idx);
                callback (num, name_converted, userdata);
            }
...
waveout_start (void) {
    ...
    {
        // force-load values
        ddb_waveformat_t tmp;
        memcpy (&tmp, &plugin.fmt, sizeof (ddb_waveformat_t));
        memset (&plugin.fmt, 0, sizeof(ddb_waveformat_t));
        pwaveout_setformat (&tmp);
    }
...
static const char settings_dlg[] =
    "property \"Buffer size (-1 to use optimal value choosen by portaudio)\" entry waveout.buffer " DEFAULT_BUFFER_SIZE_STR ";\n"
    "property \"Device name encoding\" select[3] waveout.devenc_list 0 \"ASCII / UTF-8\" cp1250 \"Defined below\";\n"
    "property \"Custom device name encoding\" entry waveout.devenc_custom \"\";\n"
;
